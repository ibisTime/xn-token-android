 ...
 */
public class WanTransactionEncoder extends TransactionEncoder {

    public static byte[] signMessage(WanRawTransaction rawTransaction,
                                     Credentials credentials) {
        byte[] encodedTransaction = encode(rawTransaction);
        Sign.SignatureData signatureData = Sign.signMessage(encodedTransaction,
    ...
    public static Sign.SignatureData createEip155SignatureData(
        ...
        return new Sign.SignatureData(v, signatureData.getR(),
                signatureData.getS());
    }

    public static byte[] encode(WanRawTransaction rawTransaction) {
        return encode(rawTransaction, null);
    }

    public static byte[] encode(WanRawTransaction rawTransaction,
                                byte chainId) {
        Sign.SignatureData signatureData = new Sign.SignatureData(chainId,
                new byte[]{}, new byte[]{});
        return encode(rawTransaction, signatureData);
    }

    private static byte[] encode(WanRawTransaction rawTransaction,
    ...
    static List<RlpType> asRlpValues(WanRawTransaction rawTransaction,
                                     Sign.SignatureData signatureData) {
        ...
        if (signatureData != null) {
            ...
            result.add(RlpString
                    .create(Bytes.trimLeadingZeroes(signatureData.getR())));
            result.add(RlpString
                    .create(Bytes.trimLeadingZeroes(signatureData.getS())));
        }
