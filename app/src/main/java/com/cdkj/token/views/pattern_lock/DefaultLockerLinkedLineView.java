...
public class DefaultLockerLinkedLineView implements ILockerLinkedLineView {
    ...
    @Override
    public void draw(@NonNull Canvas canvas, @Nullable List<Integer> hitList, @NonNull List<LockPointModel> cellBeanList, float endX, float endY, boolean isError) {
        ...
        LockPointModel first = cellBeanList.get(hitList.get(0));
        path.moveTo(first.x, first.y);
        for (int i = 1; i < hitList.size(); i++) {
            LockPointModel c = cellBeanList.get(hitList.get(i));
            path.lineTo(c.x, c.y);                                      //绘制连接线
        }

        if (((endX != 0) || (endY != 0)) && (hitList.size() < 9)) {      //绘制移动线
